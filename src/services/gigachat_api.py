...
class GigaChatService:
    ...
    def _build_user_prompt(data: dict[str, Any]) -> str:
        ...
        active_period_keys = tuple(
            key for key in DETAILED_PERIOD_KEYS if key in detailed_periods
        )
        period_names = {
        ...
        if active_period_keys:
            ...
            lines.append(
                f"Оставшиеся доступные интервалы на сегодня: {remaining_labels}."
            )
            lines.append(
                "В итоговом сообщении можно добавлять только эти интервалы и никакие другие."
            )
            lines.append(
                "Если во входных данных нет блока для времени суток, этот блок нужно полностью пропустить в готовом тексте."
            )
    ...
    def _render_strict_post(data: dict[str, Any]) -> str:
        ...
        temp_min = GigaChatService._format_temp_value(data.get("temp_min", "?"))
        temp_max = GigaChatService._format_temp_value(data.get("temp_max", "?"))
        description = GigaChatService._sentence(
            data.get("description", "переменная погода")
        )
        ...
        parts = [
            ...
            "",
            GigaChatService._build_analysis_paragraph_1(
                city, description, temp_min, temp_max
            ),
            "",
            ...
            for item in multi_day:
                ...
                parts.extend(
                    [
                        ...
                        GigaChatService._sentence(
                            item.get("description", "Без уточнений")
                        ),
                        "",
                    ]
                )
    ...
    def _build_analysis_paragraph_2(data: dict[str, Any]) -> str:
        ...
        if not detailed_periods:
            return (
                "Дальнейшее развитие погоды зависит от следующих обновлений прогноза."
            )
        ...
        first_desc = GigaChatService._sentence(
            first_period.get("description", "Без уточнений")
        )
        last_desc = GigaChatService._sentence(
            last_period.get("description", "Без уточнений")
        )
        return (
    ...
    def _pick_sticker(period: dict[str, Any], label: str) -> str:
        text = " ".join(
            str(period.get(key, "")).lower() for key in ("description", "weather")
        )
